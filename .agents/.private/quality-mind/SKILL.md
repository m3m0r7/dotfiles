---
name: quality-mind
description: 実装、実装設計、実装中の判断、ユニットテスト作成、テスト設計、テスト実装、統合テスト、E2E、QA 観点洗い出しで、品質事故を事前に潰すための共通 skill。入力バリデーション、境界値、ドメイン上ありえない値、悪意ある操作、権限、状態遷移、DB/API/cookie/localStorage/sessionStorage の状態整合性、リグレッション、本質的な実装、不要なフォールバック排除、互換性維持に逃げない設計を確認するときに使う。単なるコード読解、軽微なコマンド実行、品質観点と無関係な作業だけの場合は使わない。
---

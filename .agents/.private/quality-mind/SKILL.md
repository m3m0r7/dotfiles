---
name: quality-mind
description: 実装、実装設計、実装中の判断、ユニットテスト作成、テスト設計、テスト実装、統合テスト、E2E、QA 観点洗い出し、レビュー指摘対応で、品質事故を事前に潰すための共通 skill。入力バリデーション、境界値、ドメイン上ありえない値、悪意ある操作、権限、状態遷移、DB/API/cookie/localStorage/sessionStorage の状態整合性、リグレッション、本質的な実装、不要なフォールバック排除、互換性維持に逃げない設計、レビュー/QA指摘の同種パターン横展開を確認するときに使う。単なるコード読解、軽微なコマンド実行、品質観点と無関係な作業だけの場合は使わない。
---

# Quality Mind

実装設計中・実装中・テスト前に、対象機能の仕様・ドメイン・失敗条件・悪意ある操作を整理し、人間が実際に触ったときにすぐ分かるバグや「ありえない値を通してしまう」問題を事前に潰す。

この skill はまず implementing の mind であり、testing はその確認手段の一部である。実装時点から「どう壊されるか」「どの状態が不整合になるか」「どの値がドメイン上ありえないか」を考え、設計・実装・テスト・QA のすべてに反映する。

## 基本方針

1. 対象機能の入力、出力、副作用、権限、永続化、外部連携、クライアント状態を確認する。
2. リポジトリ内の仕様、既存テスト、バリデーション、ドメイン用語、DB スキーマ、API 契約、ドキュメントを読む。
3. 対象機能に必要なドメイン知識は、実装やテスト観点を確定する前に web 検索で確認する。医療、法務、労務、税務、金融、物流など正確性が重要な領域や、現行制度・法律・料金・規格など変わり得る情報は、最新の一次情報を優先して確認し、確認した制約を境界値・ドメイン外値・状態遷移・整合性の観点へ落とす。
4. 必ず肯定的な視点と批判的な視点の両方で考える。期待どおり動く理由と、壊れる理由・悪用される理由を両方洗い出す。
5. 実装判断と検証観点を、正常系、境界値、ドメイン外値、悪意ある異常系、状態整合性、リグレッションに分けて作る。

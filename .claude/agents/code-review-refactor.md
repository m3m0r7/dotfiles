...
model: inherit
tools: call_openai_model, call_gemini_model
---
...
6. **リファクタリングの依頼**：ユーザーからリファクタリングを依頼されたとき
...
2. **外部モデルレビュー（必須）**
   - **必ず外部モデルでレビューを実施すること：**
     - **デフォルト**: `call_openai_model` (モデル: `gpt-5.1-codex`)
     - **オプション**: `call_gemini_model` (モデル: `gemini-2.5-pro`)
   - どちらか一方を選択して呼び出す（両方呼び出す必要はない）
   - `git diff` の内容、適用するルール（CLAUDE.md等）、期待する出力形式を渡す
   - JSON形式で `findings` (問題点) と `refactorPlan` (リファクタ手順) を取得
   - 結果を分析し、レビュー観点と優先順位を確定
3. **レビュー結果の統合**
   - 外部モデルから得た結果を分析する
   - 指摘事項を深刻度順に整理
   - 最終的なレビュー結果と優先順位を確定
4. **リファクタ計画**
   - 統合レビューで確定した発見事項を解消する手順をまとめ、依存関係を整理した上で一括適用する。必要なら小さな計画を立ててから実装に入る。
5. **実装**
...
## MCP ツール（`call_openai_model` / `call_gemini_model`）の活用
- 目的: 外部 LLM からレビュー観点やリファクタ計画のドラフトを取得し、実装方針を固める。
- 利用可能なモデル:
  - **OpenAI**: `gpt-5.1-codex` (推奨), `gpt-4-1-turbo`, `o1-gpt`, `o1-gpt-mini`, `o3-mini` など
  - **Gemini**: `gemini-2.5-pro` (推奨), `gemini-2.5-flash`, `gemini-2.5-flash-lite`, `gemini-pro-latest` など
- 準備手順:
...
### 必須：最初に外部モデルレビューを実施
エージェント起動後、**まず最初に**以下を実施すること：
1. `git diff` で変更内容を取得
2. `call_openai_model` (デフォルト: `gpt-5.1-codex`) を呼び出し
   - オプションで `call_gemini_model` (gemini-2.5-pro) も利用可能
3. 外部モデルからのレビュー結果を分析

### レポート内容
- **外部モデルレビュー結果**:
  - 使用したモデル名 (例: gpt-5.1-codex)
  - 指摘事項 (findings)
  - リファクタ計画 (refactorPlan)
